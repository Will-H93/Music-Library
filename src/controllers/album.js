...
const newAlbum = async (req, res) => {
  ...
  try {
    await db.query(
      'INSERT INTO Album (album, year, artistId) VALUES (?, ?, ?)',
      [album, year, artistId]
    );
...
const readId = async (req, res) => {
  ...
  const [[album]] = await db.query(
    'SELECT Album.id, Album.album, Album.year, Artist.name, Artist.genre FROM Album LEFT JOIN Artist ON Album.ArtistId = Artist.id WHERE Album.id = ?',
    [albumId]
  );
  console.log(album);
  if (album) {
